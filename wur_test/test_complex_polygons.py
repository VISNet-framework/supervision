...
def create_image(create_image=False):
    ...
    image_circle_holes = cv2.circle(image_circle_hole.copy(), (450, 450), 25, 0, thickness=-1)
    ...
    image_circle_holes_complex_double = cv2.circle(image_circle_holes_complex_double.copy(), (320, 320), 25, 255, thickness=-1)
    ...
    if create_image:
        ...
        cv2.imwrite("circle_holes_complex_double.png", image_circle_holes_complex_double)

    return {"circle": image_circle,
...
def test_darwin_image_circle(json_name="circle.json", img_dict={}):
    ...
    check_mask_similarity(real_mask, darwin_mask, threshold=0.975) # no threshold of 1 because of small errors in manual annotations

    ## check if generated mask from conversion of mask2polyon and poly2mask is same as ground truth
    polygon_dict = dataset_darwin._detection_mask_to_darwin_polygon(real_mask, 0.0, 1.0, 0.0)
    converted_mask = detection_darwin.darwin_polygon_to_mask(polygon_dict, height=real_mask.shape[0], width=real_mask.shape[1])
